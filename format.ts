 ...
 * Based on Unicode Mathematical Alphanumeric Symbols block (U+1D400–U+1D7FF).  [oai_citation:1‡codepoints.net](https://codepoints.net/mathematical_alphanumeric_symbols?lang=en&utm_source=chatgpt.com)
 */
const styleMaps = {
  bold: {
    upper: [
...
/**
 * Convert an entire string to a single style’s Unicode equivalents.
 */
...
export function formatWithTags(input: string): string {
  return input.replace(
    ...
    (_, tag: string, content: string) => {
      ...
      if (!styleMaps[style]) return content; // unknown tag → no change
